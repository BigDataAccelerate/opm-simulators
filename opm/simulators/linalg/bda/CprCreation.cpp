...
// CprCreation<block_size>::CprCreation(BlockedMatrix *mat_)
template <unsigned int block_size>
CprCreation<block_size>::CprCreation()
{
    diagIndices.resize(1);
}

template <unsigned int block_size>
void CprCreation<block_size>::create_preconditioner_amg(BlockedMatrix *mat_) {
std::cout <<" in CprCreation<block_size>::create_preconditioner_amg(BlockedMatrix *mat_) "<< pressure_idx << std::endl;
    mat = mat_;
    cprNb = mat_->Nb;
    cprnnzb = mat_->nnzbs;
...
//         auto init_func = std::bind(&CprCreation::init_opencl_buffers, this);
...
template <unsigned int block_size>
void CprCreation<block_size>::analyzeHierarchy() {
...
std::cout << " num_levels = " << num_levels << std::endl;//Razvan
...
template <unsigned int block_size>
void CprCreation<block_size>::analyzeAggregateMaps() {
