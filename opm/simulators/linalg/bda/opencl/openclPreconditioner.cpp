...
template <unsigned int block_size>
std::unique_ptr<Preconditioner<block_size> > openclPreconditioner<block_size>::create(PreconditionerType type, int verbosity, bool opencl_ilu_parallel) {
...
template <unsigned int block_size>
bool openclPreconditioner<block_size>::analyze_matrix(BlockedMatrix *mat, [[maybe_unused]] BlockedMatrix *jacMat) {
    return analyze_matrix(mat);
}

template <unsigned int block_size>
bool openclPreconditioner<block_size>::create_preconditioner(BlockedMatrix *mat, [[maybe_unused]] BlockedMatrix *jacMat) {
    return create_preconditioner(mat);
}

#define INSTANTIATE_BDA_FUNCTIONS(n)  \
template std::unique_ptr<Preconditioner<n> > openclPreconditioner<n>::create(PreconditionerType, int, bool);         \
...
template bool openclPreconditioner<n>::create_preconditioner(BlockedMatrix *, BlockedMatrix *);
