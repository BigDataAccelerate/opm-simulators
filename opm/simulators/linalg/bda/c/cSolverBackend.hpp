...
#ifndef OPM_C_BACKEND_HEADER_INCLUDED
#define OPM_C_BACKEND_HEADER_INCLUDED
...
private:
    ...
    double* d_Avals;
    int* d_Acols, *d_Arows;        // matrix in BSR format on GPU
    ...
    std::unique_ptr<cPreconditioner<block_size> > prec;
...
//     std::shared_ptr<cl::CommandQueue> queue;
    ...
    cSolverBackend(int linear_solver_verbosity, int maxit, double tolerance, unsigned int platformID, unsigned int deviceID, bool opencl_ilu_parallel, std::string linsolver);

    /// For the CPR coarse solver
    cSolverBackend(int linear_solver_verbosity, int maxit, double tolerance, bool opencl_ilu_parallel);
